...
from tkinderIDA.trials.checkpoint import process_frame
    ...
    def video_thread():
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                """
                # Üçgen tespiti bayrağı
                triangle_detected = False

                # Kırmızı üçgen tespiti için kenar bulma
                ...
                for contour in contours:
                    ...
                    if len(approx) == 3:  # Üçgen tespiti
                        cv2.drawContours(frame, [approx], 0, (0, 255, 0), 3)
                        triangle_detected = True  # Üçgen tespit edildiğinde bayrağı değiştir

                # Kırmızı üçgen tespit edilip edilmediğini kontrol et ve mesaj yazdır
                if triangle_detected:
                    cv2.putText(frame, "Kirmizi Ucgen Tespit Edildi", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                else:
                    cv2.putText(frame, "Kirmizi Ucgen Tespit Edilmedi", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                """
                ...
                for contour in contours:
                    ...
                    if len(approx) == 3:
                        ...
                        if 10 < area < 10000:  # Üçgen alanı filtrelemesi (örneğin 10 ile 10000 arasında)
                            ...
                            if 20 < angle1 < 120 and 20 < angle2 < 120 and 20 < angle3 < 120:
                                # Üçgeni çiz
                                cv2.drawContours(frame, [approx], 0, (0, 255, 0), 3)
                ...
                if triangle_detected:
                    cv2.putText(frame, "Kirmizi Ucgen Tespit Edildi", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    # Üçgenin merkezine bir işaret koy
                    cv2.circle(frame, triangle_center, 5, (0, 0, 255), -1)
                ...
                # red_center zaten tanımlı
                x_red, y_red = triangle_center

                # Kırmızı cismin biraz yukarısında bir nokta için yeni koordinat
                y_nokta = y_red - 100  # 20 piksel yukarıda bir nokta (isteğe bağlı ayarlanabilir)
                nokta_koordinat = (x_red, y_nokta)

                # Çizimi yapalım
                cv2.circle(frame, nokta_koordinat, 5, (0, 255, 0), -1)  # Kırmızı bir nokta çiziliyor
                ...
                # x ekseni
                x_ekseni=(322,240)
                cv2.line(frame,(x_red,y_nokta),(650, y_nokta),(0,0,0),1)
